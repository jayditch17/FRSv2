...
if(isset($_POST["userID"]) && !empty($_POST["userID"])){
    ...
    if(empty($firstName_err) && empty($lastName_err) && empty($org_err) && empty($position_err) && empty($email_err) && empty($pass_err)){
        ...
        $sql = "UPDATE account_orgs SET firstName=?, lastName=?, studOrg=?, studPosition=?, studEmail=?, studPassword=? WHERE userID=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssi", $param_fname, $param_lName, $param_org, $param_position, $param_email, $param_pass, $param_userID);
    ...
    if(isset($_GET["userID"]) && !empty(trim($_GET["userID"]))){
        ...
        $sql = "SELECT * FROM account_orgs WHERE userID = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                $org = $row["studOrg"];
                $position = $row["studPosition"];
                $email = $row["studEmail"];
                $pass = $row["studPassword"];
                } else{
